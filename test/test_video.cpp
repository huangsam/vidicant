...
TEST(VideoHandlerTest, GetResolution) {
    auto mockLoader = std::make_unique<MockVideoLoader>();
    EXPECT_CALL(*mockLoader, open("test.mp4")).WillOnce(::testing::Return(true));
    EXPECT_CALL(*mockLoader, getResolution()).WillOnce(::testing::Return(std::make_pair(1920, 1080)));
...
TEST(VideoGlobalTest, GetVideoFrameCountReal) {
    int frameCount = vidicant::getVideoFrameCount("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_EQ(frameCount, 250);
}

TEST(VideoGlobalTest, GetVideoFPSReal) {
    double fps = vidicant::getVideoFPS("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_EQ(fps, 25.0);
}

TEST(VideoGlobalTest, GetVideoResolutionReal) {
    auto [width, height] = vidicant::getVideoResolution("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_EQ(width, 320);
    EXPECT_EQ(height, 176);
}

TEST(VideoGlobalTest, GetVideoDurationReal) {
    double duration = vidicant::getVideoDuration("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_EQ(duration, 10.0);
}

TEST(VideoGlobalTest, ExtractFirstFrameReal) {
    cv::Mat frame = vidicant::extractFirstFrame("/workspaces/vidicant/examples/sample.mp4");
...
TEST(VideoGlobalTest, GetVideoAverageBrightnessReal) {
    double brightness = vidicant::getVideoAverageBrightness("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_GT(brightness, 0.0);
}

TEST(VideoGlobalTest, IsVideoGrayscaleReal) {
    bool grayscale = vidicant::isVideoGrayscale("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_FALSE(grayscale);
}

TEST(VideoGlobalTest, SaveFirstFrameAsImageReal) {
    bool saved = vidicant::saveFirstFrameAsImage("/workspaces/vidicant/examples/sample.mp4", "/workspaces/vidicant/examples/test_first_frame.jpg");
    EXPECT_TRUE(saved);
    // Clean up
...
TEST(VideoGlobalTest, GetVideoMotionScoreReal) {
    double motion = vidicant::getVideoMotionScore("/workspaces/vidicant/examples/sample.mp4");
    EXPECT_GE(motion, 0.0);
}

TEST(VideoGlobalTest, GetVideoDominantColorsReal) {
    auto colors = vidicant::getVideoDominantColors("/workspaces/vidicant/examples/sample.mp4");
